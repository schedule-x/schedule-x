...
- The iCalendar plugin returned in v3.2 with Temporal support, but it assumes all timestamps are UTC. Keep the calendar timezone at `UTC` to preserve the legacy behaviour or pre-process the events if you need timezone-aware rendering.
