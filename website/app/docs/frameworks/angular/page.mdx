...
<Tabs.Tab>
  ```bash copy pnpm install @schedule-x/angular @schedule-x/calendar
  @schedule-x/theme-default @preact/signals preact ```
</Tabs.Tab>
...
<ng-template #timeGridEvent let-arg>
  <div
    style="background-color: #d6c5ea; border: 2px solid #856dc0; height: 100%; width: 100%; border-radius: 5px;
padding: 5px"
  >
...
| `listEvent`                 | The component for events used in the list view                   | `calendarEvent`, `eventInstanceInfo` |
| `listDayHeader`             | Replaces the day header in the list view                         | `date`                               |
| `listNoEvents`              | Replaces the "no events" message in the list view                | `$app`                               |
