...
<Callout type="info">
  Some of the APIs documented here are only available on the alpha version of the upcoming v2 release. You can use them by changing all
  package versions to `2.0.0-alpha.0`, except for your framework adapter (for example @schedule-x/vue).

  A stable version is planned for release in the last week of September 2024.
</Callout>
