...
| COUNT                                        | Number                                                                                                                                                                               | Yes (unless UNTIL is provided) |
| INTERVAL                                     | Number                                                                                                                                                                               | No                             |
| BYDAY - compatible with `DAILY` and `WEEKLY` | `MO`, `TU`, `WE`, `TH`, `FR`, `SA`, `SU`                                                                                                                                             | No                             |
| BYMONTHDAY  - compatible with `MONTHLY`      | Positive integer 1-31                                                                                                                                                                | Yes (unless COUNT is provided) |
...
const event = calendar.eventsService.get(123);

// Get all
const events = calendar.eventsService.getAll();

// Remove an event with id 123
calendar.eventsService.remove(123);
```
