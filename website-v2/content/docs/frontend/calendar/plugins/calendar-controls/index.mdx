...
Set the day boundaries of the calendar. Both `start` and `end` must be whole hours in the format `HH:00` (e.g. `06:00`, `18:00`). Half-hours such as `08:30` are not supported.
...
Set the resources of the calendar.
...
Get the current resources of the calendar.
