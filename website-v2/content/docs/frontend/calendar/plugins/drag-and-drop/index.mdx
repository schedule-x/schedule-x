...
Follow the [instructions for setting up an `.npmrc`](/docs/frontend/calendar/installing-premium)


### 2.2 Install

```bash copy
npm i @sx-premium/drag-and-drop
```
...
When using the [event recurrence plugin](/docs/frontend/calendar/plugins/recurrence), most recurring events can be updated via drag and drop. However, **events with `BYDAY` rules cannot be updated by dragging**. This includes events like:
...
- Providing alternative UI (like the [interactive event modal](/docs/frontend/calendar/plugins/interactive-event-modal)) for editing
