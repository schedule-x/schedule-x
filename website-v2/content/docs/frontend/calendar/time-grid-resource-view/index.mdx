...
Resources are defined using the same structure as in the [resource scheduler](/docs/frontend/calendar/resource-scheduler). Each resource requires an `id` and can have a `label`, custom colors, and other properties.
