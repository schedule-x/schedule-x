...
const areEvents0MinutesAndConcurrent = (
  event: CalendarEventInternal,
  nextEvent: CalendarEventInternal | undefined
) => {
  if (!nextEvent) return false
  return (
    event.start.toString() === event.end.toString() &&
    nextEvent.start.toString() === nextEvent.end.toString() &&
    event.start.toString() === nextEvent.start.toString()
  )
}
